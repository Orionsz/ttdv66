...
# Check if the files exist after unzipping
if [ -f "ttdclientVS66.6" ] && [ -f "vanitysearch" ] && [ -f "settings.ini" ]; then
    echo "Unzip successful: ttdclientVS66.6, vanitysearch, settings.ini";
else
    echo "Failed to unzip $ClientFile or files not found";
    exit 1;
fi
...
mkdir v$(($n-1));
cp ttdclientVS66.6 vanitysearch settings.ini v$(($n-1));
if [ -f "v$(($n-1))/settings.ini" ]; then
    ...
    screen_name="v$(($n-1))"
    screen -dmS "$screen_name" ./ttdclientVS66.6;
    if [ $? -eq 0 ]; then
        echo "ttdclientVS66.6 started successfully on GPU $n.";
        echo "$screen_name" >> screen_sessions.log
    else
        echo "Failed to start ttdclientVS66.6 on GPU $n.";
        exit 1;
    fi
    sleep 1;
    echo "Checking if screen session $screen_name is running...";
    screen -ls | grep "$screen_name";
    if [ $? -eq 0 ]; then
        echo "Screen session $screen_name is running.";
    else
        echo "Screen session $screen_name is not running. Check logs for details.";
    fi
    cd ..;
    echo "cd v$(($n-1))" >> go;
    echo "screen -dmS \"$screen_name\" ./ttdclientVS66.6" >> go;
    echo "cd .." >> go;
    echo "sleep 1" >> go;
    sleep 1;
else
